...
namespace Subasta.Client.Common.Images
	{
		...
		{
			public void LoadImages(IDeck deck,  ImageList imageListCards, Size size,string resourceNamespace=null)
			{
				if(resourceNamespace==null) resourceNamespace = string.Format("{0}.Content.Images", Assembly.GetAssembly(GetType()).GetName().Name);
